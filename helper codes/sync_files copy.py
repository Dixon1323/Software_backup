...
def create_partial_report_with_shift_signs(date_str):
    ...
    # Step 1: load original template and replace text placeholders (date fields)
    try:
    ...
    # Save temp docx with only text replacements (do not replace shift placeholders)
    tmp_text_docx = os.path.join(OUTPUT_DIR, f"temp_text_{date_str}_{uuid.uuid4().hex}.docx")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    try:
        doc.save(tmp_text_docx)
    except Exception as e:
        log(f"Failed to save temporary text-replaced document: {e}")
        return None
    ...
    for placeholder, img_path in placeholder_image_map.items():
        if img_path:
            ...
            try:
                ...
                final_image_map[placeholder] = img_path  # fallback
        else:
    ...
    # Step 3: inject images into the temp_text_docx, producing final docx
    final_docx = os.path.join(OUTPUT_DIR, f"Daily_Report_{date_str}_partial.docx")

    final_docx_safe = safe_save_docx(final_docx)
    mapping_for_xml["(date)"] = date_str
    mapping_for_xml["(date_with_month)"] = human_date
    
    inject_images_into_docx(tmp_text_docx, final_docx_safe, placeholder_image_map)
    ...
    # Step 4: optional post-processing with python-docx
    try:
        doc_final = Document(final_docx)
        force_arial(doc_final)
        doc_final.save(final_docx)
    except Exception as e:
        log(f"Post-processing final document failed (non-fatal): {e}")

    # cleanup temp_text_docx
    try:
    ...
    log(f"Saved partial: {final_docx}")
    return final_docx
